...
def ddpg_low_dim_state(path):
    ...
    config.task_fn = lambda **kwargs: ReacherV1('ReacherV1', path, log_dir)
    ...
    config.network_fn = lambda: DeterministicActorCriticNet(
        config.state_dim, config.action_dim,
        actor_body=FCBody(config.state_dim, (128, 128), gate=F.tanh),
        critic_body=TwoLayerFCBodyWithAction(
            config.state_dim, config.action_dim, (128, 128), gate=F.tanh),
